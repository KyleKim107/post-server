...
public class PostService {
    ...
    public PostService(PostRepository postRepository, UploaderService uploaderService, KafkaTemplate kafkaTemplate, ObjectMapper objectMapper) {
    ...
    private SocialPost updateUploaderName(SocialPost post) { // 업로더명 가져오는 함수 생성
    ...
    public List<SocialPost> getAllPostsByUploaderId(int uploaderId) {
        return postRepository.findByUploaderId(uploaderId)
                .stream().map(SocialPost::fromPostEntity)
                .map(this::updateUploaderName)
                .toList();
    }
    ...
    public SocialPost createPost(PostRequest post) {
        ...
        PostActivity message = new PostActivity(
                savedPost.getUploaderId(),
                savedPost.getUploadDatetime(),
